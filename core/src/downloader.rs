...
impl StateManagerTask {
    fn get_work(&mut self) -> Option<(ChunkState, Arc<AtomicU64>)> {
        ...
        if let Some(c) = self
            ...
            .chunks
            .iter()
            .find(|c| !self.active_chunks.contains_key(&c.id) && c.current <= c.end)
        {
...
impl Downloader {
    ...
    pub async fn download(
        ...
        if !supports_range {
            self.progress.emit_log(
                "Server does not support multipart downloads. Falling back to single thread."
                    .to_string(),
            );
            num_threads = 1;
        }
        ...
        let (download_state, is_resume) =
            StateManager::load_or_init(&state_file_path, url, content_length, supports_range);

        if !is_resume {
            let file =
                File::create(&file_path).map_err(|e| format!("Failed to create file: {}", e))?;
            file.set_len(content_length)
                .map_err(|e| format!("Failed to allocate disk space: {}", e))?;
        } else if !file_path.exists() {
            return Err(
                "Target file is missing. Please delete the .boltfetch file and start over.".into(),
            );
        }
        ...
        tokio::spawn(async move {
            ...
            while let Some(msg) = state_rx.recv().await {
                ...
                if needs_save || last_save.elapsed() >= save_interval {
                    if let Err(e) =
                        StateManager::save(&sm_task.state_file_path, &sm_task.download_state)
                    {
            ...
            let task = tokio::spawn(async move {
                loop {
                    ...
                    let (chunk, atomic_end) = {
                        ...
                        if state_tx_clone
                            .send(StateMessage::RequestWork { reply: reply_tx })
                            .await
                            .is_err()
                        {
                            break;
                    ...
                    let response = loop {
                        let range_header = format!(
                            "bytes={}-{}",
                            current_offset,
                            atomic_end.load(Ordering::Relaxed)
                        );
                        match client_clone
                            .get(&url_clone)
                            ...
                            .await
                        {
                                    ...
                                    || res.status() == StatusCode::SERVICE_UNAVAILABLE
                                {
                                    if retries >= max_retries {
                                        ...
                                        let _ = state_tx_clone
                                            .send(StateMessage::ChunkComplete {
                                                chunk_id: chunk.id,
                                            })
                                            .await;
                                        return; // Give up
                                    ...
                                    *err_flag.lock().await = format!("HTTP {}", res.status());
                                    let _ = state_tx_clone
                                        .send(StateMessage::ChunkComplete { chunk_id: chunk.id })
                                        .await;
                                    return;
                                }
                                break res;
                            ...
                            Err(e) => {
                                ...
                                retries += 1;
                                tokio::time::sleep(tokio::time::Duration::from_secs(1 << retries))
                                    .await;
                            }
                        }
                    };
                    ...
                    while let Some(chunk_bytes) = chunk_stream.chunk().await.unwrap_or(None) {
                        ...
                        if buffer.len() >= 2 * 1024 * 1024
                            || network_offset >= atomic_end.load(Ordering::Relaxed)
                        {
                            let f = file_clone.clone();
                            let offset = current_offset;
                            let data_to_write =
                                std::mem::replace(&mut buffer, Vec::with_capacity(2 * 1024 * 1024));

                            let write_result =
                                tokio::task::spawn_blocking(move || -> std::io::Result<u64> {
                                    let mut chunk_offset = 0;
                                    while chunk_offset < data_to_write.len() {
                                        let written = f.write_at(
                                            &data_to_write[chunk_offset..],
